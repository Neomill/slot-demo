...
import { spinSubStyle, spinTitleStyle } from './styles';

/**
 * The primary action button: the spin-button art with "SPIN" / "HOLD FOR AUTO"
...
export class SpinButton extends Container {
  ...
  constructor(texture: string, width: number, height: number, onPress: () => void) {
    super();
    ...
    const sub = new Text({ text: 'HOLD FOR AUTO', style: spinSubStyle() });
    sub.anchor.set(0.5);
    sub.y = 16;

    this.addChild(sprite, title, sub);
